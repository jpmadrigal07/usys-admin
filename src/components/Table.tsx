...
const Table: React.FC<IProps> = ({ data, columns, additionalCol }) => {
  const tableColumns = useMemo(() => (columns ? columns : COLUMNS), []);
  const tableData = useMemo(() => (data ? data : MOCK_DATA), []);
  ...
  return (
    <div className="text-sm">
      ...
      <div className="py-2">
        <Input className="w-40" placeholder="Search" size="sm" />
      </div>

      <div className="my-5 text-dark text-sm">
        <table
          className="w-full divide-y divide-solid divide-gray "
          {...getTableProps()}
        >
          <thead>
          ...
          <tbody
            {...getTableBodyProps()}
            className="divide-y divide-solid divide-gray"
          >
            {page.map((r: any) => {
              prepareRow(r);
              return (
                <tr className="hover:bg-gray" {...r.getRowProps()}>
                  {r.cells.map((c: any) => (
                    <td
                      className={`${
                        c.column.id === "Edit" && "flex flex-row-reverse"
                      } my-1 px-2`}
                      {...c.getCellProps()}
                    >
                      {c.render("Cell")}
